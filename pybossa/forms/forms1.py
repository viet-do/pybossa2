...
class RegisterForm(Form):
    ...
    err_msg = lazy_gettext("Please Enter City")
    city = TextField(lazy_gettext('City'), [validators.Length(min=2,max=100,message=err_msg)])
    states = [('',' Choose a State'),('AL','Alabama'),('AK','Alaska'),('AZ','Arizona'),('AR','Arkansas'),('CA', 'California'),('CO', 'Colorado'),
    ...
    err_msg = lazy_gettext("Please Choose a State")
    state = SelectField('State', choices=states, validators=[validators.Required(message=err_msg)]) # viet
    #############################################################viet
    ...
    consent = BooleanField(lazy_gettext('I accept receiving emails from EasyText'),false_values=("False", "false", '', '0', 0))
    #############################################################viet
